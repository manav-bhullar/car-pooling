...
function DriftTracker({ targetPositions, onDriftChange }) {
  ...
  useEffect(() => {
    ...
    const checkDrift = () => {
      ...
      const expectedBounds = L.latLngBounds(t.map((p) => [p[0], p[1]])).pad(
        0.5,
      );
  ...
  return (
    ...
    >
      ...
      >
        ...
        {sortedStops.map((s) => {
          const isMyStop = myRideRequestId
            ? s.rideRequestId === myRideRequestId
            : true;
          return (
            <Marker
              key={s.stopOrder}
              position={[s.lat, s.lng]}
              icon={getMarkerIcon(s.type, isMyStop)}
            >
              <Popup>
                {isMyStop
                  ? s.type === "PICKUP"
